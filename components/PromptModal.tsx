        ...
        <div className="p-6 bg-white">
          ...
          <form onSubmit={handleSubmit}>
            <textarea 
              ...
              disabled={isLoading}
              className={`w-full h-24 resize-none bg-slate-50 border-2 border-slate-200 rounded-xl p-4 font-medium text-slate-700 focus:outline-none focus:ring-4 transition-all placeholder:text-slate-400 mb-4 ${isCreate ? 'focus:border-sky-400 focus:ring-sky-100' : 'focus:border-amber-400 focus:ring-amber-100'}`}
              autoFocus
            />
            ...
            <div className="mb-4">
                ...
                <button
                    ...
                    className={`w-full py-3 border-2 border-dashed rounded-xl flex items-center justify-center gap-2 font-bold transition-colors ${imageBase64 ? 'border-emerald-400 bg-emerald-50 text-emerald-600' : 'border-slate-300 text-slate-400 hover:border-slate-400 hover:bg-slate-50'}`}
                >
                    {imageBase64 ? (
                        <>
                            <Sparkles size={18} /> Image Selected (Click to change)
                        </>
                    ) : (
                        <>
                            <span>📷</span> Upload Reference Photo
                        </>
                    )}
                </button>
            </div>
